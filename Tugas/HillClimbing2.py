...
def display_board(solution):
    if solution is not None:
        ...
        for i, j in enumerate(solution):
            matrix[7-i][j] = 1 # ubah baris menjadi 7-i
        l = [i+1 for i in range(len(solution))]
        plt.figure(figsize=(5,5))
        sns.heatmap(matrix, linewidths=.8, cbar=False, cmap='Set3', xticklabels=l, yticklabels=l[::-1]) # ubah urutan label y
        plt.show()
        print("Solution: ")
        for i, j in enumerate(solution):
            print("[{}, {}]".format(j, 7-i)) # ubah baris menjadi 7-i
    else:
